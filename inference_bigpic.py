...
from data import SSDD_CLASSES as labelmap
from data import *
#from data import VOC_ROOT, VOCAnnotationTransform, VOCDetection, BaseTransform
from data import SSDDDetection, SSDDAnnotationTransform, SSDD_ROOT,BaseTransform
...
def draw_clsdet(img,cls_dets,vis_thresh):
    ...
    for i in range(len(cls_dets)):
        #x1,y1,x2,y2=int(cls_dets[ii][0]),int(cls_dets[ii][1]),int(cls_dets[ii][2]),int(cls_dets[ii][3])
...
def save_bbox2txt(bboxes,save_name):
    pass
class Timer(object):
    """A simple timer."""
    def __init__(self):
...
def eval_results(annot_dir,det_path,imagesetfile,clss,overthre,conf_thre):
...
if __name__ == '__main__':
    ...
    annotdir=os.path.join(args.data_dir,'labels_dota')
    ...
    import pdb;pdb.set_trace()
    eval_results(annotdir,det_path,imagenames,cfg['classname'],args.iou_thre,args.conf_thre)
