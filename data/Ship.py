...
Ship_ROOT = '/data/03_Datasets/CasiaDatasets/MixShip512_100'
...
class ShipDetection(data.Dataset):
    ...
    def __init__(self, root,
                 image_sets=['Ship'],split='train',
                 transform=None, target_transform=ShipAnnotationTransform(),
                 dataset_name='Ship'):
        self.root = root
        ...
        for line in open(osp.join(rootpath,'{}.txt'.format(split))):
                #import pdb;pdb.set_trace()
                ship_id=line.strip()
                self.ids.append((rootpath,ship_id))
                self.image_indexes.append(ship_id)
